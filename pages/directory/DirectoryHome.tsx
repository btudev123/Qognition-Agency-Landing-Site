...
const DirectoryHome: React.FC = () => {
  ...
  const pageTitle = category 
    ? `${category.split('-').map(w => w.charAt(0).toUpperCase() + w.slice(1)).join(' ')} Tools` 
    ...
    <>
      <SEO 
        ...
        schemaData={{
          ...
          url: `https://qognitionagency.com/directory${category ? '/' + category : ''}`,
          mainEntity: {
             "@type": "ItemList",
             "itemListElement": allTools.map((tool, index) => ({
                "@type": "ListItem",
                "position": index + 1,
                "url": `https://qognitionagency.com/directory/${currentCategorySlug}/${tool.id}`,
                "name": tool.name
             }))
          }
        }}
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 mb-16">
            {allTools.length > 0 ? (
                allTools.map((tool, index) => (
                    <Link to={`/directory/${currentCategorySlug}/${tool.id}`} key={tool.id} className="group">
                        <motion.div 
                            initial={{ opacity: 0, y: 20 }}
